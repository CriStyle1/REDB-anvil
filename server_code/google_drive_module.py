...
# ==================================================================
# ANVIL UPLINK CONNECTION (Corrected Paths)
# ==================================================================
import anvil.server
import json

# Decorate the functions you want to call from Anvil
@anvil.server.callable
def get_sheet_manager_instance():
    """
    This function initializes the GoogleSheet manager.
    We use a simple caching mechanism to avoid re-initializing on every call.
    """
    if not hasattr(anvil.server, 'sheet_manager_instance'):
        print("--- INITIALIZING GOOGLE SHEET MANAGER (Uplink) ---")
        ...
        sheet_manager.settings_file = "drive_module_settings.json" # CORRECTED PATH
        sheet_manager.settings = sheet_manager._load_settings()
        sheet_manager.load_data()
        anvil.server.sheet_manager_instance = sheet_manager
    return anvil.server.sheet_manager_instance

@anvil.server.callable
def calculate_utilities_anvil(ap_code, include_rent):
    sm = get_sheet_manager_instance()
    return sm.calculate_and_format_utilities(ap_code, include_rent)

@anvil.server.callable
def generate_master_report_anvil(ap_code):
    sm = get_sheet_manager_instance()
    return sm.generate_master_report(ap_code)

@anvil.server.callable
def get_apartment_data_anvil(ap_code):
    sm = get_sheet_manager_instance()
    return sm.get_apartment_data(ap_code)

@anvil.server.callable
def format_email_log_anvil(ap_code):
    sm = get_sheet_manager_instance()
...
def connect_to_anvil():
    ...
    try:
        with open('anvil_secrets.json', 'r') as f: # CORRECTED PATH
            secrets = json.load(f)
        uplink_key = secrets.get('uplink_key')
        if not uplink_key:
            print("❌ Uplink key not found in anvil_secrets.json")
            return

        anvil.server.connect(uplink_key)
        print("✅ Anvil Uplink connected. Waiting for calls...")
        anvil.server.wait_forever()

    except FileNotFoundError:
        print("❌ anvil_secrets.json not found. Please create it with your Uplink key.")
    except Exception as e:
        print(f"❌ An error occurred during Uplink connection: {e}")
